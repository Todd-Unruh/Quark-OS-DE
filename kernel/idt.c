/* =============================================================================
 * Quark-OS kernel/idt.c
 * Interrupt Descriptor Table + exception and IRQ dispatchers
 * ============================================================================= */
...
/* Forward declarations for all ISR/IRQ stubs from isr_stubs.asm */
...
static const char *exception_names[] = {
    ...
    "Unknown",                 "x87 FPU Error",        "Alignment Check",
...
// Ensure the compiler knows about your mouse handler
/* Called from irq_common_stub for hardware interrupts */
void irq_common_handler(registers_t *regs) {
    // 1. Map the absolute interrupt vector index (32-47) down to an IRQ number (0-15)
    uint8_t irq_no = regs->int_no - 32;

    // 2. Call the registered dynamic pointer handler if it exists
    if (irq_handlers[irq_no] != 0) {
        void (*handler)(registers_t *) = irq_handlers[irq_no];
        handler(regs);
    }

    // 3. Complete End-Of-Interrupt PIC notification cycles
    if (regs->int_no >= 40) {
